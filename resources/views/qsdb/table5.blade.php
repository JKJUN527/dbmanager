...
@section('content')
    <div class="container body">
        <div class="main_container">
                @include('components.indexNav',['activeIndex'=>0,'activeIndexSecend'=>4])
                    ...
                    <div class="">
                        ...
                        <div class="row">
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                <div class="x_panel">
                                    <div class="x_title">
                                        <h2>组合式互感器 <small>在这里查询、处理组合式互感器</small></h2>
                                    ...
                                    <div class="x_content">
                                        ...
                                        <table id="datatable-buttons" class="table table-striped table-bordered">
                                            ...
                                            <tbody>
                                            @foreach($data['mixtransformer'] as $item)
                                                <tr>
                                                    <td>{{$item->start_table_num}}</td>
                                                    <td>{{$item->vender_name}}</td>
                                                    <td>{{$item->model}}</td>
                                                    <td>{{$item->ct_variable}}</td>
                                                    <td>{{$item->pt_variable}}</td>
                                                    <td>{{$item->region}}</td>
                                                    <td>{{$item->company}}</td>
                                                    <td>{{$item->project}}</td>
                                                    <td>{{$item->num}}</td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">组合式互感器</h4>
                </div>
                <div class="modal-body">
                    <form class="form-horizontal form-label-left">
                        ...
                        <div class="form-group">
                            <label>CT变比</label>
                            <input type="text" class="form-control" name="ct_variable" id="ct_variable" placeholder="请输入CT变比(xx/xx)">
                            <label class="error" for="ct_variable"></label>
                        </div>
                        <div class="form-group">
                            <label>PT变比</label>
                            <input type="text" class="form-control" name="pt_variable" id="pt_variable" placeholder="请输入PT变比(xx/xx)">
                            <label class="error" for="pt_variable"></label>
                        </div>
                        <div class="form-group">
                            <label>单位</label>
...
@section('custom-script')
    ...
    <script>
        $('#pick_time').click(function () {
            $(location).attr('href', "/db/table5?date=" + $('#reservation').val());
        });
        $('button[name=clear_time]').click(function () {
            $(location).attr('href', "/db/table5");
        });
        //验证号段是否符合规定
        ...
        $('#modify-post').click(function () {
            ...
            var ctvariable = $('input[name=ct_variable]');
            var ptvariable = $('input[name=pt_variable]');
            ...
            formData.append('ctvariable', ctvariable.val());
            formData.append('ptvariable', ptvariable.val());
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if(result.status == 200){
                        ...
                        $('#ct_variable').val(result.data.ct_variable);
                        $('#pt_variable').val(result.data.pt_variable);
        ...
        $("#search_tablenum").click(function () {
            ...
            $(location).attr('href', "/db/table5?tablenum=" + table_num);
        });
